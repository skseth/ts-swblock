...
async function removeRegistrationForDomain(domain: string): Promise<SWPreferences> {
...
export async function removeRegistrationForServiceWorker(
  ...
  if (scriptURL in prefs.regs) {
    ...
    throw new Error(`removeRegistrationForServiceWorker: Attempt to delete non-existent script ${scriptURL} from domain ${domain}`)
  }
}
